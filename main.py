...
class eventNode:
  def __init__(self, note, Date = datetime.date.today(), prev = None, next = None):
    self.note = note
    self.date = Date
    self.prev = None
    self.next = None
...
class student:
  ...
  def newNote(self):
    ...
    if self.head != None: # Checks whether list is empty or not
        self.head.prev = newNode # old head's previous pointer will refer to newly created node
      ...
      self.head = newNode
      self.tail = newNode
      newNode.prev = None # There's only one element so both pointers refer to null
